...
- **Xong khi:** URL Vercel mở được trang chủ ✅ — `https://shigakogen.vercel.app` chạy thật, `/blog` load đúng dữ liệu từ Supabase Cloud. Domain riêng `https://shigakogen.site` đã set làm Production trên Vercel, DNS đúng, deployment READY — verify qua Vercel API/MCP (`domains` có cả apex + www, deployment target=production READY). **Không verify được bằng curl trực tiếp** — mạng sandbox này có firewall Fortinet chặn hẳn domain lạ chưa phân loại (trả về trang chặn của Fortinet, không phải lỗi site) — **bạn cần tự mở trình duyệt xác nhận**.
...
- **Đã verify chạy thật (2026-08-21)** — 4 secret (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_SITE_URL`, `SUPABASE_ACCESS_TOKEN`) đã set qua `gh secret set`. `ci.yml` xanh trên cả `uat` và `main` (2 job `frontend` + `edge-functions`). `deploy-functions.yml` xanh qua `workflow_dispatch` (đã thêm trigger này để test không cần đợi đổi `supabase/functions/**`), redeploy 5 Edge Functions thành công lên production.
  - Fix kèm theo: `pnpm typecheck` trước đó chỉ pass trên máy dev vì `.next/types` đã có sẵn từ các lần build trước — trên checkout sạch của CI thì chưa có, gây lỗi `Cannot find name 'LayoutProps'`. Sửa bằng cách đổi script thành `next typegen && tsc --noEmit` (`package.json`).
