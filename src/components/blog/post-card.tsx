...
export function PostCard({ post }: { post: PostListItem }) {
  return (
    <article className="border-border border-b py-6 first:pt-0 last:border-b-0">
      <Link
        href={`/blog/${post.slug}`}
        className="focus-visible:outline-accent-strong group rounded-sm outline-none focus-visible:outline-2 focus-visible:outline-offset-2"
      >
        <h2 className="group-hover:text-accent-strong text-xl font-[650] tracking-[-0.02em] transition-colors">
          {post.title}
        </h2>
      </Link>
